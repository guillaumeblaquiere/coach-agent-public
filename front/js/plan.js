...
export function initPlanModule(coachBackendURL, userEmail, addMessageToChat) {
    ...
    const localeFlag = document.getElementById('locale-flag');
    ...
    // --- Locale State ---
    let currentLocale = 'en-EN'; // Default locale
    const supportedLocales = ['en-EN', 'fr-FR'];
    ...
    function getLocalizedString(value, locale) {
        ...
        // If the backend already provided a pre-localized string, return it directly.
        if (typeof value === 'string') {
            return value;
        }
        // If the value is a map of translations, find the best match.
        if (typeof value === 'object' && value !== null) {
            // Try the current locale, then English as a fallback, then the first available one.
            return value[locale] || value['en-EN'] || Object.values(value)[0] || "";
        }
    ...
    async function loadPlanTemplate() {
        // Caching is now handled by the calling function (loadPlan) which clears the template on locale change.
        try {
            const response = await fetch(`${coachBackendURL}/api/v1/plan-templates/default?locale=${currentLocale}`);
            if (!response.ok) {
    ...
    async function loadPlan() {
        ...
        try {
            // If the template is not loaded (e.g., first load or locale change), fetch it.
    ...
    function displayPlan(planTemplate, dailyPlanData) {
        ...
        for (const categoryId in planTemplate.categories) {
            ...
            header.innerHTML = `<h3>${getLocalizedString(category.name, currentLocale)}</h3>`;
            ...
            if (category.drills) {
                for (const drillId in category.drills) {
                    ...
                    drillName.textContent = getLocalizedString(drill.name, currentLocale);
                    drillElement.appendChild(drillName);

                    const drillDescText = getLocalizedString(drill.description, currentLocale);
                    if (drillDescText) {
                        const drillDesc = document.createElement('div');
                        drillDesc.classList.add('drill-desc');
                        drillDesc.textContent = drillDescText;
    ...
    // --- Functions to handle locale ---
    function updateFlagIcon(locale) {
        if (!localeFlag) return;
        // Maps the full locale to a simple country code for the flag image.
        const flagMap = {
            'fr-FR': 'fr',
            'en-EN': 'gb' // Great Britain flag for English
        };
        const flagCode = flagMap[locale] || 'gb'; // Default to gb
        // MODIFIED: Point to an online SVG flag icon provider
        localeFlag.src = `https://flagcdn.com/${flagCode}.svg`;
        localeFlag.alt = `${locale} flag`;
    }

    function initLocale() {
        const browserLang = navigator.language || 'en-EN';
        // Find a supported locale, e.g., 'fr' from 'fr-FR'
        const matchedLocale = supportedLocales.find(l => browserLang.startsWith(l.split('-')[0]));
        currentLocale = matchedLocale || 'en-EN'; // Default to English if no match
        localeSelector.value = currentLocale;
        updateFlagIcon(currentLocale);
    }

    async function handleLocaleChange() {
        currentLocale = localeSelector.value;
        updateFlagIcon(currentLocale);
        console.log(`Locale changed to: ${currentLocale}`);
        // Force a refetch of the template by clearing the cached version
        currentPlanTemplate = null;
        // Reload the entire plan view with the new locale
        await loadPlan();
    }
    ...
    localeSelector.addEventListener('change', handleLocaleChange);
